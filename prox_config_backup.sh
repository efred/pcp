...
# set vars (die nachfolgende Variable "savefolders" muss zwingend auf Zeile 13 bleiben)
savefolders="/etc/*, /var/lib/pve-cluster/*, /root/*, /var/spool/cron/*, /usr/share/kvm/*.vbios"

# Settings for E-Mail-Notifitacion:
rm ~/prox_config_backup.log
echo "Error" > ~/prox_config_backup.log
...
function clean_up {
    echo "Cleaning up"
    # rm -rf $_tdir #ausgeklammert FJ
    rmdir $_tdir # erstellt FJ
}
...
# description # Ausselektioniert, damit ohne Rückfrage gesichert wird
...
rm ~/prox_config_backup.log
echo "Success" > ~/prox_config_backup.log